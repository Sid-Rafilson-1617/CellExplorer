...
[~, filename, ~] = fileparts(filename); % remove extension, if present

%Buzcode outputs
% bz_sleepstatepath = fullfile(savefolder,[recordingname,'.SleepState.states.mat']);
bz_sleepstatepath = fullfile(savefolder,[filename,'.SleepState.states.mat']);
...
if isempty(filename)
    ...
    chInfo = hackInfo('basepath',basepath,'filename',filename);
end
...
[SleepScoreMetrics,StatePlotMaterials] = ClusterStates_GetMetrics_km(...
...
[ints,idx,MinTimeWindowParms] = ClusterStates_DetermineStates_km(SleepScoreMetrics);
...
StatesToEpisodes_km(SleepState,basepath,filename);

display(['Sleep Score ',filename,': Complete!']);
...
if ~noPrompts
    ...
    switch str
        case {'Y','y',''}
            TheStateEditor([basepath,filesep,filename])
        case {'N','n'}
        otherwise
