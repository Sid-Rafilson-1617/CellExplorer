...
if isempty(filename) % default
    ...
    if strcmp(filename(end-3:end),'.lfp') || strcmp(filename(end-3:end),'.dat')
        filename = filename(1:end-4); % remove extension
    end
    matfilename = fullfile(basepath,[filename,'.EMGFromLFP.LFP.mat']);
end
...
if ~isempty(chInfo)
    ...
    if isempty(filename) % default
        ...
        lfpFile = checkFile('filename',filename,'fileType','.lfp');
        lfpFile = [lfpFile.folder filesep lfpFile.name];
    end
end
