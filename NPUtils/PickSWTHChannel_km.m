...
if isempty(filename) % default
    figfolder = fullfile([basePath, filesep, recordigname '_StateScoreFigures']);
    matfilename = fullfile(basePath,[recordingname,'.SleepScoreLFP.LFP.mat']);
else 
    figfolder = fullfile([basePath, filesep, filename '_StateScoreFigures']);
    [~, recordingname, ~] = fileparts(filename);
    matfilename = fullfile(basePath, [recordingname, '.SleepScoreLFP.LFP.mat']);
end
...
%session = bz_getSession('basepath',basePath,'datFile',recordingname);
...
allLFP = bz_GetLFP(usechannels,'basepath',basePath,'lfpFile',filename,...
...
%tstart = tic;
%addAttachedFiles(gcp,{fullfile('private','hartigansdiptest_ss.m')})
...
swthLFP = bz_GetLFP([SWchanID,THchanID],'basepath',basePath,'lfpFile',filename,...
...
normTHspec = bz_NormToRange(THmeanspec,[0 numusedchannels.*0.6]);
    ...
    subplot(5,1,3)
        ...
        if saveFiles
            saveas(chanfig,[figfolder,filesep,filename,'_SWTHChannels'],'jpeg')
        end
