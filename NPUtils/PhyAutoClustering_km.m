...
% precompute: best channel for each spike in indx
members = ch_indx(clu(indx) + 1);   % column vector of channel IDs

% unique shanks as a row vector
shanks = unique(channel_shanks)';
base_clu = max(clu);                % freeze base so it doesn't change in loop

for jj = 1:numel(shanks)
    j = shanks(jj);
    shank_channels = ch_indx2{j};   % list of channels on this shank
    
    % --- membership test without ismember ---
    mask = false(size(members));
    for k = 1:numel(shank_channels)
        mask = mask | (members == shank_channels(k));
    end
    indx22 = find(mask);
    % ----------------------------------------

    new_cluster_id = base_clu + j;
    clu2(indx(indx22)) = new_cluster_id;
    artifact_clusters = [artifact_clusters, new_cluster_id];
end
