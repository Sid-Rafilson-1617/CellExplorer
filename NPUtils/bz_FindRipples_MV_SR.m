function [ripples] = bz_FindRipples_MV(varargin)
...
addParameter(p,'durations',[20 200],@isnumeric) % [30 100] %[15 250] %[15 100]
...
addParameter(p,'passband',[130 250],@isnumeric) % [130 200]
...
elseif isnumeric(varargin{1}) % if first arg is filtered LFP
    ...
    passband = p.Results.passband;
    EMGThresh = p.Results.EMGThresh;
    filename = p.Results.filename; % NP added
...
if ~isempty(noise)
    if length(noise) == 1 % you gave a channel number
       noiselfp = bz_GetLFP(p.Results.noise,'basepath',basepath,'lfpFile',basename);%currently cannot take path inputs. Fixed SR
...
if EMGThresh
    ...
    %session = bz_getSession(basepath, 'datFile', basename);
    %EMGfilename = fullfile(basepath,[session.FileName '.EMGFromLFP.LFP.mat']);
    EMGfilename = fullfile(basepath,[basename '.EMGFromLFP_iEEG.mat']); %%% CHANGED FOR iEEG %%% [basename '.EMGFromLFP.LFP.mat']
    if exist(EMGfilename)
        load(EMGfilename)   %should use a bz_load script here
    else
        [EMGFromLFP] = bz_EMGFromLFP_km(basepath,'samplingFrequency',10,'savemat',true,'noPrompts',true,'filename',filename); % 'savemat' previously false
    end
    excluded = logical(zeros(size(ripples,1),1));
    for i = 1:size(ripples,1)
...
if plotType == 1 || plotType == 2
    figName = fullfile(basepath, [filename '.ripples_plot.png']);
    saveas(gcf, figName);              % or exportgraphics(gcf, figName, 'Resolution',300)
end
