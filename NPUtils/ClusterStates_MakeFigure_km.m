function ClusterStates_MakeFigure_km(SleepState,basePath,filename,noprompts) 
...
EMG = motiondata; % must change EMGhist, EMGhistbins, and EMGthresh on original ClusterStates_MakeFigure

%%
%Figure locations
figloc = fullfile(basePath,[filename '_StateScoreFigures']); 
if ~exist(figloc,'dir')
    mkdir(figloc)
end
...
clusterfig = figure('visible','off');
	subplot(8,1,[1:2])
        ...
        set(gca,'XTickLabel',{})
        ylabel({'swLFP','f (Hz)'})
        title(regexprep([recordingname,': State Scoring Results'], '_', '\\_'));
    ...
    subplot(8,1,4)
   	...
   	subplot(6,1,6)
	    ...
	    cd(figloc)
        saveas(clusterfig,[figloc '/' filename '_SSResults'],'jpeg')
...
saveas(gcf,[figloc '/' filename '_SSCluster2D'],'jpeg')
...
if noprompts
	...
	saveas(gcf,[figloc '/' filename '_SSCluster3D'],'jpeg')
...
%         title('REM stateintervalserval Durations')
%         
%         saveas(gcf,[figloc,filename,'_stateintervalsdur'],'jpeg')
% %         
%     
